...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        elseif (in_array('ROLE_EMPLOYEE', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('siv_emp_homepage'));
        elseif (in_array('ROLE_SECURITY', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('siv_security_homepage'));
